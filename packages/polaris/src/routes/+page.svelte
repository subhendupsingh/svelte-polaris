<script lang="ts">
	...
	const handleSelectionChange = (selectionType: string, toggleType: boolean, selection?: any, sortOrder?: number)=> {
...
<Page>
	<IndexTable hasZebraStriping selectedItemsCount={selectedItems.length} onSelectionChange={handleSelectionChange} selectable={true} headings={[{ title: "ID"}, { title: "Order"}, { title: "Date"}, { title: "Customer"}, { title: "Total"}]} itemCount={orders.length} resourceName={resourceName}>
		{#each orders as order}
			<IndexTable.Row selected={selectedItems.includes(Number(order.id))} onClick={() => console.log(order)} id={order.id} position={orders.indexOf(order)}>
